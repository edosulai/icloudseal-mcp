---
name: icloudseal
description: "Use for iCloud Mail, Notes, Calendar, or sealed MCP."
---
...
- desktop deferred `mcp__icloudseal__icloud_*` (double underscore) via
  `tool_describe` / `tool_call`

If neither skill nor MCP is attached, tell the user to run
`icloudseal-mcp setup` then
...
From a git checkout, `mcp-wrapper.sh` is the same Python entry.
...
1. `setup_mcp(server='icloudseal', action='enable')` injects them into the
...
Notes updates that must keep native tables, headings, or lists **cannot**
go through `icloud_prepare_notes_update` / `applescript.update_note`. That
