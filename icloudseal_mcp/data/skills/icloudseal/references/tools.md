...
| CLI | Notes |
| --- | --- |
| `setup [--platform P] [--project] [--force] [--json]` | Install default hosts + print MCP attach snippets |
| `install-skill [--platform P] [--project] [--force]` | Copy `skills/icloudseal/` to agent skill dirs |
| `uninstall-skill [--platform P] [--project] [--force]` | Remove only the icloudseal skill copy |

Default platforms are `copilot,claude,codex,agents,hermes`.
Pass `--platform all` or a comma list (`claude,codex,…`) explicitly.
Home dirs that are whole-directory symlinks (dotfiles checkouts) are
skipped unless `--force`. `mcp-wrapper.sh` runs `install-skill` so hosts
pick up `/icloudseal` without a manual copy. stdout stays reserved for
MCP stdio.
